...
int binary_serialize_entity(uint8_t* buf, int buf_size, int entity) {
    ...
    if (image)    flags |= BCREATE_HAS_IMAGE;
    if (physics)  flags |= BCREATE_HAS_PHYSICS;
    if (collider) flags |= BCREATE_HAS_COLLIDER;
    if (player)   flags |= BCREATE_HAS_PLAYER;
    if (enemy)    flags |= BCREATE_HAS_ENEMY;
    if (health)   flags |= BCREATE_HAS_HEALTH;
    if (weapon)   flags |= BCREATE_HAS_WEAPON;
    if (item)     flags |= BCREATE_HAS_ITEM;
    if (waypoint) flags |= BCREATE_HAS_WAYPOINT;
    if (particle) flags |= BCREATE_HAS_PARTICLE;
    if (light)    flags |= BCREATE_HAS_LIGHT;
    if (sound)    flags |= BCREATE_HAS_SOUND;
    if (ammo)     flags |= BCREATE_HAS_AMMO;
    if (anim)     flags |= BCREATE_HAS_ANIMATION;
    if (door)     flags |= BCREATE_HAS_DOOR;
    if (joint)    flags |= BCREATE_HAS_JOINT;
    if (vehicle)  flags |= BCREATE_HAS_VEHICLE;
    if (text)     flags |= BCREATE_HAS_TEXT;

    uint8_t* ptr = buf;
    int rem = buf_size;
    int n;

    // Write create_flags (4 bytes)
    if (rem < 4) return 0;
    memcpy(ptr, &flags, 4);
    ptr += 4; rem -= 4;
    ...
    // --- Player ---
    // No creation parameters needed (PlayerComponent_add takes only entity)
...
// Macro to check remaining space
#define CHECK(need) do { if (ptr + (need) > end) return 0; } while(0)

    // --- Image ---
    if (flags & BCREATE_HAS_IMAGE) {
        Filename filename;
        ...
        ImageComponent* image = ImageComponent_add(entity, filename, width, height, (Layer)layer);
        image->alpha = alpha;
        image->shine = shine;
    }

    // --- Physics ---
    if (flags & BCREATE_HAS_PHYSICS) {
        CHECK(4 + 4 + 4 + 4);
    ...
    // --- Player ---
    if (flags & BCREATE_HAS_PLAYER) {
        PlayerComponent_add(entity);
    }

    // --- Enemy ---
    if (flags & BCREATE_HAS_ENEMY) {
        CHECK(4*7 + 1 + 1 + 4 + 4 + 4);
    ...
    if (flags & BCREATE_HAS_HEALTH) {
        ...
        if (!sn) return 0; ptr += sn;
        CHECK(4 * DAMAGE_COUNT);
        HealthComponent* health = HealthComponent_add(entity, (int)hp, dead_image, decal, die_sound);
        health->max_health = (int)max_hp;
        for (int i = 0; i < DAMAGE_COUNT; i++) {
            health->damage_factor[i] = read_f32(&ptr);
        }
    }

    // --- Weapon ---
    if (flags & BCREATE_HAS_WEAPON) {
        ...
        WeaponComponent* weapon = WeaponComponent_add(entity, fire_rate, (int)damage, (int)shots,
    ...
    if (flags & BCREATE_HAS_ITEM) {
        ...
        ItemComponent* item_comp = ItemComponent_add(entity, (int)size, (int)price, name);
    ...
    // --- Waypoint ---
    if (flags & BCREATE_HAS_WAYPOINT) {
        WaypointComponent_add(entity);
    }

    // --- Particle ---
    if (flags & BCREATE_HAS_PARTICLE) {
        CHECK(1);
        uint8_t type = read_u8(&ptr);
        if (type == PARTICLE_NONE) {
            ...
            ParticleComponent* particle = ParticleComponent_add(entity, angle, p_spread,
            ...
            uint8_t loop = read_u8(&ptr);
            float rate = read_f32(&ptr);
            ParticleComponent* particle = ParticleComponent_add_type(entity, (ParticleType)type, start_size);
    ...
    // --- Ammo ---
    if (flags & BCREATE_HAS_AMMO) {
        CHECK(1 + 2);
        uint8_t type = read_u8(&ptr);
        int16_t size = read_i16(&ptr);
        AmmoComponent* ammo = AmmoComponent_add(entity, (AmmoType)type);
        ammo->size = (int)size;
    }

    // --- Animation ---
    if (flags & BCREATE_HAS_ANIMATION) {
        CHECK(2 + 4 + 1 + 4);
        int16_t frames = read_i16(&ptr);
        float framerate = read_f32(&ptr);
        uint8_t play_once = read_u8(&ptr);
        float wind_factor = read_f32(&ptr);
        AnimationComponent* anim = AnimationComponent_add(entity, (int)frames);
    ...
    // --- Door ---
    if (flags & BCREATE_HAS_DOOR) {
        CHECK(2 + 1 + 1);
        int16_t price = read_i16(&ptr);
        uint8_t locked = read_u8(&ptr);
        uint8_t key = read_u8(&ptr);
        DoorComponent* door = DoorComponent_add(entity, (int)price);
        door->locked = locked;
        door->key = (int)key;
    }

    // --- Joint ---
    if (flags & BCREATE_HAS_JOINT) {
        CHECK(2 + 4*4);
        ...
        JointComponent* joint = JointComponent_add(entity, (int)parent, min_length, max_length, strength);
        joint->max_angle = max_angle;
    }

    // --- Vehicle ---
    if (flags & BCREATE_HAS_VEHICLE) {
        CHECK(4 + 1 + 4 + 4 + 4);
        float max_fuel = read_f32(&ptr);
        uint8_t size = read_u8(&ptr);
        float acceleration = read_f32(&ptr);
        float max_speed_v = read_f32(&ptr);
        float turning = read_f32(&ptr);
        int seat_count = (size < 4) ? size : 4;
        CHECK(seat_count * 8);
        VehicleComponent* vehicle = VehicleComponent_add(entity, max_fuel);
        vehicle->size = (int)size;
        vehicle->acceleration = acceleration;
        vehicle->max_speed = max_speed_v;
        vehicle->turning = turning;
        for (int i = 0; i < seat_count; i++) {
            vehicle->seats[i].x = read_f32(&ptr);
            vehicle->seats[i].y = read_f32(&ptr);
        }
    }

    // --- Text ---
    if (flags & BCREATE_HAS_TEXT) {
...
#define SKIP_CHECK(need) do { if (ptr + (need) > end) return 0; } while(0)
    ...
    // --- Image ---
    if (flags & BCREATE_HAS_IMAGE) {
        // length-prefixed string + 4+4+1+4+4
        if (ptr >= end) return 0;
        uint8_t slen = *ptr;
        ptr += 1 + slen;
    ...
    if (flags & BCREATE_HAS_WEAPON) {
        ...
        // length-prefixed string (sound name)
        if (ptr >= end) return 0;
        uint8_t slen = *ptr;
        if (ptr + 1 + slen > end) return 0;
        ptr += 1 + slen;
